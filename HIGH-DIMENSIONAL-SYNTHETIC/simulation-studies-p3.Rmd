...
This simulation setting emulates
...
but focuses on unbalanced settings.
...
stored in the file `sur-candes-3.RData` and can be reproduced running
the script
...
df$xl <- cut(df$x, breaks = c(0, seq(1, p, by = (p - 1) / 5)), labels = c("Intercept", paste0("beta==", c("3", "3/2", "0", "-3/2", "-3"))))
